...
- 实现一个 MCP Server（`demos.03_mcp.server`），暴露 **get_weather** 工具，内部请求 wttr.in 获取天气。
- 提供 **list_tools_client**：通过 stdio 连接该 Server，调用 list_tools 并打印工具列表。
- 可在 Cursor 中配置 `.cursor/mcp.json`（args: `["-m", "demos.03_mcp.server"]`，cwd 为项目根），或使用 MCP Inspector 连接并测试 call_tool。

本 Demo 为 **Server 端**，需单独启动；学习站内「运行 Demo」仅提示启动方式。实际调用可在终端运行 `python -m demos.03_mcp.list_tools_client` 或通过 Cursor/Inspector 体验。
