...
本讲介绍 **OpenClaw Skill**：一种在 OpenClaw 生态中扩展 Agent 能力的规范。通过编写 **SKILL.md** 等约定，可以把自定义能力（如规则、提示词、工具说明）注入到 OpenClaw Gateway 与 Agent 中。本仓库在 **demos/11_skill/** 下提供示例 **demo-skill**，便于理解 Skill 的结构与用法；若需被 OpenClaw 加载，需将 Skill 目录放到项目根下的 **skills/** 中。
...
- 在 **demos/11_skill/** 下提供 **demo-skill** 示例，内含 **SKILL.md** 及必要说明。
- 不涉及命令行可执行脚本；学习站内「运行 Demo」会提示：本主题为 OpenClaw Skill 示例，请阅读 **demos/11_skill/README.md**，将 demo-skill 复制到 **skills/** 后启动 OpenClaw Gateway 进行体验。

若需被 OpenClaw 加载：将 **demos/11_skill/demo-skill** 复制到项目根目录的 **skills/demo-skill**，然后启动 Gateway。
