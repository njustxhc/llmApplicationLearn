...
- **MCP 工具**：勾选「使用 MCP 工具」后，若用户问天气，会连接 **demos.04_mcp.server** 调用 get_weather，将结果注入后再生成回复。
...
- MCP Server：默认连接 `demos.04_mcp.server`；可通过环境变量 `MCP_SERVER_MODULE`、`MCP_SERVER_CWD` 覆盖。

本 demo 为学习项目中的「综合应用」版本，使用 **core.llm**、**core.rag_doc** 和 **demos.04_mcp**。
